...
- `es/` → Elasticsearch 后端（N6，`ElasticsearchSession` 用 `java.net.http.HttpClient` + `ElasticsearchProtocol` 纯逻辑）；**禁止 import compose/coroutines**
...
2. **应用/窗口级全局偏好 → `<dataDir>/*.properties`（`app/settings/*Prefs`）**：theme、window、
   tree-expand、editor（编辑器字体/字号）、keymap（扩展业务功能快捷键覆盖）、redis（按连接的 DB/pattern/类型）。都是标量、无查询/级联需求；
