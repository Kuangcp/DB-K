...
**发布验证（P8）**一条主线，以及若干可选深化项（集群级目录 / 行锁 / 保存的查询）。
...
- **诊断入口** ✅（P9）：设置窗口「通用设置 → 诊断」显示日志 / 数据目录路径并一键用文件管理器打开；无桌面环境回落「复制路径 + 行内提示」。
...
优先级逻辑：P4–P7、P9 已落地；剩余 P8 发布闭环（Deb + 干净环境验证）与若干可选深化
（集群级目录 / 行锁 / 保存的查询）。
剩余阶段状态：P4 ✅ / P5 ✅ / P6 ✅（按组懒加载）/ P7 部分（预览 ✅，行锁未做）/
P8 未开始 / P9 部分（诊断入口 ✅，保存的查询未做）。
...
### P9 诊断与易用性小项（诊断入口 ✅ / 保存的查询未做）

1. **「打开日志 / 数据目录」入口** ✅：设置窗口「通用设置」底部「诊断」分区显示日志目录路径 +
   「打开日志目录」「打开数据目录」按钮。用 `Desktop.getDesktop().open(dir)`（`app/core/DesktopOpen.kt`，
   预先 `isDesktopSupported` / `isSupported(OPEN)` 双重判定 + 吞异常）；无桌面环境时**复制路径到剪贴板 +
   行内提示**（设置窗口是独立窗口，主窗口 Toast 会被遮住，故用行内提示而非 Toast）。
   日志目录统一由 `AppPaths.logsDirectory()` 解析（`-Ddbk.logDir` 覆盖），与 `SessionLogWriter` 共用，不再重复。
